...
export class UserService {
  ...
  async login({ email, password }: LoginDTO) {
    ...
    const refreshToken = jwt.sign({ userId: user.id }, REFRESH_SECRET, { expiresIn: "1h" });
  ...
  async refreshToken(refreshToken: string) {
    try {
      ...
      const tokens = {
        ...
        refreshToken: jwt.sign({ userId: decoded.userId }, REFRESH_SECRET, { expiresIn: "1h" })
      };
      await this.#prisma.user.update({ where: { id: decoded.userId }, data: { refreshToken: tokens.refreshToken } });
      return tokens;
    } catch {
