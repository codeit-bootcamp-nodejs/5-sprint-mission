...
import { UserLikeListReqValidator } from "./req.validator/user/user.like.List.req.dto.js";
...
import { UserLikeArticlesResDto } from "./res.dto/user/user.like.articles.dto.js";
import { UserLikeProductsResDto } from "./res.dto/user/user.like.products.dto.js";
...
export class UserController {
  ...
  signInUserController = async (req, res, next) => {
    ...
    const { accessToken, authenticatedUser } =
      await this.#services.auth.signInUser(userReqDto);
  ...
  getUserProductsController = async (req, res, next) => {
    ...
    const { user, products } =
      await this.#services.user.getUserProducts(userReqDto);
    const resDto = new UserProductsResDto({ user, products });
    return res.json(resDto);
  };

  getUserLikeProductsController = async (req, res, next) => {
    const userReqDto = new UserLikeListReqValidator({
      userId: req.userId,
      query: req.query,
    }).validate();

    const likeProducts =
      await this.#services.user.getUserLikeProducts(userReqDto);
    const resDto = new UserLikeProductsResDto(likeProducts);

    return res.json(resDto);
  };

  getUserLikeArticlesController = async (req, res, next) => {
    const userReqDto = new UserLikeListReqValidator({
      userId: req.userId,
      query: req.query,
    }).validate();
    ...
    const likeArticles =
      await this.#services.user.getUserLikeArticles(userReqDto);
  ...
  updateUserPasswordController = async (req, res, next) => {
    ...
    const user =
      await this.#services.user.updatePasswordUser(UserPasswordReqDto);
    const resDto = new UserResDto(user);
    return res.json(resDto);
  };
  ...
  refreshTokensController = async (req, res, next) => {
    const { accessToken, user } = await this.#services.auth.refreshTokens(
