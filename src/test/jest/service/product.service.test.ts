...
import {
  ...
  ProductCommandServiceType,
} from "../../../02-application/command/service/product.command.service";
import {
  createProductQueryService,
  ProductQueryServiceType,
} from "../../../02-application/query/service/product.query.service";
...
describe("Product Service 단위 테스트", () => {
  ...
  describe("상품 생성 (createProduct)", () => {
    ...
    test("새 상품을 성공적으로 생성하고 알림을 발행해야 합니다", async () => {
      ...
      (mockNotificationRepo.create as jest.Mock).mockResolvedValue(
        mockNotification,
      );
  ...
  describe("상품 단건 조회 (getProduct)", () => {
    test("ID로 상품을 성공적으로 조회해야 합니다", async () => {
      ...
      (mockProductQueryRepo.findById as jest.Mock).mockResolvedValue(
        mockProduct,
      );
  ...
  describe("상품 수정 (updateProduct)", () => {
    ...
    test("작성자 본인이면 상품을 성공적으로 수정해야 합니다 (가격 변경 없음)", async () => {
      ...
      (mockProductRepo.findById as jest.Mock).mockResolvedValue(
        existingProduct,
      );
  ...
  describe("상품 좋아요 (likeProduct)", () => {
    ...
    test("다른 사람의 상품에 좋아요를 누르면 알림을 전송해야 합니다", async () => {
      ...
      (mockProductLikeRepo.toggle as jest.Mock).mockResolvedValue(
        mockProductLike,
      );
      (mockNotificationRepo.create as jest.Mock).mockResolvedValue(
        mockNotification,
