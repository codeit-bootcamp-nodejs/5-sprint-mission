...
export const createProductQueryService = (
  ...
  const getProduct = async (id: string) => {
    ...
    if (cachedProduct) {
      ...
      for (let i = 0; i < 5; i++) {
        lock = await redisExternal.setIfNotExist(`lock:product:${id}`, ".", 3);
        if (lock) {
          // 1명 통과
          const foundProduct = await productQueryRepository.findById(id);
          await redisExternal.set(key, JSON.stringify(foundProduct));
...
export type ProductQueryServiceType = ReturnType<
  typeof createProductQueryService
>;
