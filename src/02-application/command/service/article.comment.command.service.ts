...
export const createArticleCommentCommandService = (
  ...
  const createArticleComment = async (dto: ArticleCommentDto) => {
    ...
    if (userId !== articleEntity.userId) {
      ...
      const notification = await notificationCommandRepository.create(notifcationEntity);
  ...
  const updateArticleComment = async (dto: ArticleCommentDto) => {
    ...
    // 댓글 조회
    const foundArticleComment = await articleCommentCommandRepository.findById(commentId);
  ...
  const deleteArticleComments = async (
    ...
    // 댓글 조회
    const articleComment = await articleCommentCommandRepository.findById(commentId);
    if (!articleComment) {
      throw BusinessException({
        type: BusinessExceptionType.DATA_NOT_FOUND
      })
