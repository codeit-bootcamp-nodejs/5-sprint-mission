...
export class NotificationController extends BaseController {
  ...
  getMyNotificationsController: ControllerHandler = async (req, res, next) => {
    const reqDto = this.validateOrThrow(
      getMyNotificationsReqSchema.safeParse({
        userId: req.userId,
        ...req.query,
      }),
    );
    ...
    const notifications =
      await this._notificationService.getMyNotifications(reqDto);
  ...
  getUnreadCountController: ControllerHandler = async (req, res, next) => {
    const reqDto = this.validateOrThrow(
      getUnreadCountReqSchema.safeParse({
        userId: req.userId,
      }),
    );
