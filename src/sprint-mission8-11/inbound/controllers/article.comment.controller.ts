...
import {
  createArticleCommentReqSchema,
  deleteArticleCommentReqSchema,
  getArticleCommentReqSchema,
  updateArticleCommentReqSchema,
...
export class ArticleCommentController extends BaseController {
  ...
  createArticleCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const reqDto = this.validateOrThrow(
      createArticleCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.params,
        ...req.body,
      }),
  ...
  getArticleCommentListController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const reqDto = this.validateOrThrow(
      getArticleCommentReqSchema.safeParse({
        ...req.params,
        ...req.query,
      }),
    );
    ...
    const getCommentListResDto = new GetArticleCommentListResDto(
      getCommentList,
    );
    return res.json(getCommentListResDto);
  };

  updateArticleCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const reqDto = this.validateOrThrow(
      updateArticleCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.params,
        ...req.body,
      }),
  ...
  deleteArticleCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const reqDto = this.validateOrThrow(
      deleteArticleCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.params,
      }),
    );
