...
import {
  ...
  getProductCommentReqSchema,
  updateProductCommentReqSchema,
...
export class ProductCommentController extends BaseController {
  ...
  createProductCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const resDto = this.validateOrThrow(
      createProductCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.params,
        ...req.body,
      }),
  ...
  getProductCommentListController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const resDto = this.validateOrThrow(
      getProductCommentReqSchema.safeParse({
        ...req.query,
        ...req.params,
      }),
    );
    ...
    const getCommentListResDto = new GetProductCommentListResDto(
      getCommentList,
    );
    return res.json(getCommentListResDto);
  };

  updateProductCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const resDto = this.validateOrThrow(
      updateProductCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.body,
        ...req.params,
      }),
  ...
  deleteProductCommentController: ControllerHandler = async (
    req,
    res,
    next,
  ) => {
    const resDto = this.validateOrThrow(
      deleteProductCommentReqSchema.safeParse({
        userId: req.userId,
        ...req.params,
      }),
    );
