...
import { IArticleCommandRepo } from "../../../port/repo/command/article/article.command.repo.interface";
import { IUserLikesArticleCommandRepo } from "../../../port/repo/command/like/user-likes-article.command.repo.interface";

export class ArticleCommandService {
  constructor(
    private readonly _articleRepo: IArticleCommandRepo,
    private readonly _userLikesArticleRepo: IUserLikesArticleCommandRepo,
  ) {}
  async getArticle(dto: GetArticleDto): Promise<PersistArticleEntity> {
    ...
    if (!foundArticle) {
      throw new BusinessException({
        type: BusinessExceptionType.ARTICLE_NOT_EXIST,
      });
    }
  ...
  async likeArticle(dto: GetLikedArticlesDto): Promise<void> {
    ...
    const likeArticleEntity = UserLikesArticleEntity.createNew({
      userId: dto.userId,
  ...
  async createArticle(dto: CreateArticleDto): Promise<PersistArticleEntity> {
    ...
    if (foundArticle) {
      throw new BusinessException({
        type: BusinessExceptionType.ARTICLE_ALREADY_EXIST,
      });
    }
  ...
  async updateArticle(dto: UpdateArticleDto): Promise<PersistArticleEntity> {
    ...
    if (dto.userId !== foundArticle.userId) {
      throw new BusinessException({
        type: BusinessExceptionType.UNAUTHORIZED_ARTICLE_OWNER,
      });
    }
    foundArticle.update({
      title: dto.title,
