...
import { NewArticleEntity, PersistArticleEntity } from "../../../entity/article.entity";
...
export interface IArticleRepo {
  ...
  findArticleLike(userId: string, articleId: string): Promise<PersistArticleEntity | null>;
  findUserLikeArticles(userId: string, offset: number, limit: number): Promise<PersistArticleEntity[] | null>;
  findArticleList(offset: number, limit: number, orderBy: { field: ArticleKeys, sort: Sort }): Promise<PersistArticleEntity[]>;
