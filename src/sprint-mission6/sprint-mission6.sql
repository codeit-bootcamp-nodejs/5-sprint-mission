...
SELECT od.date, COUNT(DISTINCT od.id) AS order_count, SUM(od_d.quantity) AS total_quantity
FROM order_details AS od_d
    JOIN orders AS od ON od_d.order_id = od.id
GROUP BY od.date
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
 SELECT od.date, count(DISTINCT od.id) AS total_orders, SUM(p.price * od_d.quantity) AS total_amount
 FROM orders AS od
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
        ...
        ```
*/
SELECT p_t.name, p.size, p.price, od_d.quantity, (p.price * od_d.quantity) AS total_amount
FROM orders AS od
    JOIN order_details AS od_d ON od.id = od_d.order_id
    JOIN pizzas AS p ON p.id = od_d.pizza_id
    JOIN pizza_types AS p_t ON p_t.id = p.type_id
WHERE od.id = 78
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size, SUM(p.price * od.quantity)
FROM order_details AS od
...
SELECT p_t.name, SUM(p.price * od.quantity)
FROM order_details AS od
    JOIN pizzas AS p ON p.id = od.pizza_id
    JOIN pizza_types AS p_t ON p_t.id = p.type_id
GROUP BY p_t.name
