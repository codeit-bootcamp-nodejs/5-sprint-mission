import { NewArticleCommentEntity, PersitstArticleCommentEntity } from "../../../domain/entity/comment/article-comment.entity";
...
export class ArticleCommentRepo extends BaseRepo implements IArticleCommentRepo {
  async findCommentById(id: number): Promise<PersitstArticleCommentEntity | null> {
    const articleComment = await this._prisma.articleComment.findUnique({
      where: { id },
    });
    return articleComment ? ArticleCommentMapper.toPersistEntity(articleComment) : null;
  };
  ...
  async findCommentList(
    ...
    return commentList.map((comment) => ArticleCommentMapper.toPersistEntity(comment));
  }

  async create(entity: NewArticleCommentEntity): Promise<PersitstArticleCommentEntity> {
