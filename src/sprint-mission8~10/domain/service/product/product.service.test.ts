...
import { NewTagEntity, PersistTagEntity, TagEntity } from "../../entity/tag.entity";
...
describe("product service 유닛 테스트", () => {
  ...
  describe("여러 상품 정보 가져오기 테스트", () => {
    ...
    test("sort가 recent면 updatedAt desc로 조회한다", async () => {
      ...
      (mockProductRepo.findProductList as jest.Mock).mockResolvedValue(fakeProducts);

      const result = await productService.getProductList(dto);

      expect(mockProductRepo.findProductList).toHaveBeenCalledWith(
        0,
        5,
  ...
  describe("한 상품 좋아요 누르기 테스트", () => {
    ...
    test("상품이 존재하면 좋아요를 생성한다", async () => {
      (mockProductRepo.findProductById as jest.Mock).mockResolvedValue({} as PersistProductEntity);

      const mockNewLikes = {} as UserLikesProductEntity;
      jest.spyOn(UserLikesProductEntity, "createNew").mockReturnValue(mockNewLikes);

      await productService.likeProduct(dto);

      expect(mockUserLikesProductRepo.create).toHaveBeenCalledWith(mockNewLikes);
    });
  });
  ...
  describe("상품 수정 테스트", () => {
    ...
    test("정상적으로 상품을 수정한다 (가격 변경 없음)", async () => {
      ...
      (mockProductRepo.update as jest.Mock).mockResolvedValue(mockePersistProduct);
    ...
    test("가격이 변경되었지만 좋아요 유저가 없으면 이벤트를 발생시키지 않는다", async () => {
      (mockProductRepo.findProductById as jest.Mock).mockResolvedValue(mockePersistProduct);
  ...
  describe("상품 삭제 테스트", () => {
    ...
    test("상품 작성자가 맞으면 삭제한다", async () => {
      (mockProductRepo.findProductById as jest.Mock).mockResolvedValue(mockPersistProduct);
