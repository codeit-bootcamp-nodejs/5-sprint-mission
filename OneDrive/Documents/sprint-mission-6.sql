...
FROM 
    orders 
WHERE   
    date='2025-03-12' AND EXTRACT(HOUR FROM time) < 12;
...
FROM    
    pizza_types 
WHERE category::text IN ('Cheese','Chicken');
...
SELECT 
    pizza_id, SUM(quantity) AS "total_count"
FROM 
    order_details
GROUP BY
    pizza_id
...
FROM 
    order_details
JOIN pizzas ON
    order_details.pizza_id = pizzas.id
GROUP BY 
    order_details.pizza_id, 
...
GROUP BY
    date, pizza_id
HAVING COUNT(order_id) != SUM(quantity);
...
SELECT
    pizzas.size, SUM(price * quantity)
FROM
    order_details
JOIN pizzas ON 
    order_details.pizza_id = pizzas.id
