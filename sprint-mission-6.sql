...
SELECT pizza.id, SUM(pizza.quantity * pizza.price) AS total_revenue
FROM order_details JOIN pizza ON order_details.pizza_id = pizza.id
GROUP BY pizza.id;
...
/*
    1. 피자별(`pizzas.id` 기준) 판매 수량 순위에서 피자별 판매 수량 상위에 드는 베스트 피자를 10개를 조회해 주세요. `pizzas`의 모든 컬럼을 조회하면서 각 피자에 해당하는 판매량을 `total_quantity`라는 이름으로 함께 조회합니다.
        ...
        ```
*/
SELECT p.*, SUM(or.quantity) AS total_quantity
FROM pizzas AS p JOIN order_details AS or ON p.id = or.pizza_id
GROUP BY p.id
...
SELECT 
  ...
  or.quantity AS quantity,
  (p.price * or.quantity) AS total_amount
FROM pizzas AS pizza
  JOIN order_details AS or ON p.id = or.pizza_id
  JOIN pizza_types AS pt ON p.type_id = pt.id
WHERE or.order_id = 78;
...
/*    
    4. `order_details`와 `pizzas` 테이블을 JOIN해서 피자 크기별(S, M, L) 총 수익을 계산하고, 크기별 수익을 출력하세요.
        ...
        ```
*/
SELECT p.size, SUM(p.price * or.quantity)
FROM order_details AS or JOIN pizzas AS p ON p.id = or.pizza_id
GROUP BY p.size
ORDER BY SUM(p.price * or.quantity) DESC;
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT pt.name, SUM(p.price * or.quantity)
FROM order_details AS or 
  JOIN pizzas AS p ON p.id = or.pizzas_id
