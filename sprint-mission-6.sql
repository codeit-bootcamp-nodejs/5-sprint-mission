...
SELECT date, count(*) as order_count from orders
GROUP BY date
HAVING count(*) >= 80
...
SELECT d.pizza_id, sum(d.quantity * p.price)  as total_revenue from pizzas as p 
JOIN order_details as d on p.id = d.pizza_id
GROUP BY d.pizza_id;

-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date, count(DISTINCT d.order_id) as order_count, sum(d.quantity) as total_quantity 
...
from pizzas as p join order_details as d 
on p.id = d.pizza_id
GROUP BY p.id
...
/*
    2. `orders` 테이블에서 2025년 3월의 일별 주문 수량을 `total_orders`라는 이름으로, 일별 총 주문 금액을 `total_amount`라는 이름으로 포함해서 조회하세요.
        ...
        ```
*/
SELECT o.date, count(DISTINCT o.id) as total_orders, SUM(d.quantity * p.price) AS total_amount
FROM orders as o join order_details as d
ON o.id = d.order_id join pizzas as p
on d.pizza_id = p.id
GROUP BY o.date
HAVING o.date >= '2025-03-01' AND o.date <= '2025-03-31'
order by date;
...
from orders as o join order_details as d 
ON o.id = d.order_id join pizzas as p on p.id = d.pizza_id
join pizza_types as t on p.type_id = t.id
WHERE o.id = 78;
...
from order_details as d join pizzas as p on p.id = d.pizza_id
GROUP BY p.size
...
/*    
    5. `order_details`, `pizzas`, `pizza_types` 테이블을 JOIN해서 각 피자 종류의 총 수익을 계산하고, 수익이 높은 순서대로 출력하세요.
        ...
        ```
*/
SELECT t.name, sum(d.quantity * p.price)
from order_details as d join pizzas as p on p.id = d.pizza_id
join pizza_types as t on p.type_id = t.id
GROUP BY t.name
ORDER BY sum(d.quantity * p.price) DESC;
