...
import { validateUpdateMe, validateChangePassword } from "../middlewares/validator/validate.user.js";
...
router.patch("/me", authMiddleware, validateUpdateMe, async (req, res, next) => {
  try {
...
router.patch("/me/password", authMiddleware, validateChangePassword, async (req, res, next) => {
    try {
      const { id: userId } = req.user;
