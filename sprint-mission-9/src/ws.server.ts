    ...
    this.io.use(async (socket, next) => {
      try {
        ...
        if (!ACCESS_TOKEN_SECRET) {
          return next(new Error("서버 설정 오류: ACCESS_TOKEN_SECRET_KEY가 없습니다."));
        }
        ...
        try {
          ...
          if (
            ...
            typeof decoded.userId !== "number"
             ...
             return next(new Error("토큰 페이로드에 유효한 사용자 ID가 없습니다."));
          }
          ...
          return next(new Error(`유효하지 않은 Access Token입니다. (${error.message})`));
        }
